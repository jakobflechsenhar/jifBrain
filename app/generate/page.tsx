...
function estimateCost(text: string, imageCount: number): string {
  const textTokens = Math.ceil(text.length / 4)
  const imageTokens = imageCount * TOKENS_PER_IMAGE
  const inputCost = (textTokens + imageTokens) * INPUT_COST_PER_M / 1_000_000
  const outputCost = ESTIMATED_OUTPUT_TOKENS * OUTPUT_COST_PER_M / 1_000_000
...
export default function GeneratePage() {
  ...
  const costEstimate = useMemo(
    () => estimateCost(text, imageFiles.length),
    [text, imageFiles.length]
  )
  const hasInput = text.trim().length > 0 || imageFiles.length > 0
  ...
  async function handleGenerate() {
    if (!hasInput) { setError('Add some text or images to generate from.'); return }
    setGenerating(true)
    setError('')
    setCards(null)
    ...
    setCards(json.cards.map((c: { question: string; answer: string }) => ({ ...c, selected: true })))
    setGenerating(false)
  }
              ...
              {imagePreviews.map((src, i) => (
                <div key={i} className="relative rounded-lg overflow-hidden" style={{ width: 72, height: 56 }}>
                  ...
                  <button onClick={() => removeImage(i)}
                    ...
                    style={{ backgroundColor: '#3d1a1a', color: '#f87171', fontSize: 9 }}>✕</button>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="flex flex-col gap-3 mb-6">
            {cards.map((card, i) => (
              <div key={i} className="rounded-2xl p-4 transition-opacity"
                ...
                <textarea value={card.answer} onChange={e => updateCard(i, 'answer', e.target.value)} rows={3}
                  placeholder="Answer"
